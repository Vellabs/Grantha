...
async fn generate_article(
    ...
    state.agent.render_topic(&topic).await.map_err(|e| e.to_string())
}
...
fn load_full_graph(state: State<AppState>, query: String) -> Result<GraphData, String> {
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    let mut sn = conn.prepare("SELECT id, label, desc, x, y FROM nodes WHERE query = ?").map_err(|e| e.to_string())?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            c.execute("CREATE TABLE IF NOT EXISTS nodes (id TEXT, label TEXT, desc TEXT, query TEXT, x REAL, y REAL, PRIMARY KEY(id, query))", []).map_err(|e| e.to_string())?;
